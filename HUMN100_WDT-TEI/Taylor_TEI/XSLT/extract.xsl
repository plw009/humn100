...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//text//persName | //text//placeName | //text//objectName | //text//date">
    ...
    <xsl:function name="jc:createCSV" as="xs:string" >
      ...
      For those which might have multiple instances, provide a separator; -->
        ...
        <xsl:variable name="ref"><xsl:value-of select="$node/@ref/jc:csvEscapeDoubleQuotes(.)"/></xsl:variable>
        
        
        <!-- Assemble the output $terminal, all our variables separated with value of $sep, followed by the $terminal -->   
        <xsl:variable name="output"><xsl:value-of select="$terminal"/><xsl:value-of select="$tag, $elementContent, $type, $ref" separator="{$sep}"/><xsl:value-of select="$terminal"/></xsl:variable>    
